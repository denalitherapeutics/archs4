...
#' found in `datadir`. **In order for this to work** you have to download
#' the approprate human and mouse gtf files from ensembl and save them in
#' `datadir`.
#'
...
create_augmented_feature_info <- function(datadir = getOption("archs4.datadir"),
  ...
  for (s in c("mouse", "human")) {
    ...
    tx.fn <- file.path(datadir, paste0(s, "_transcript_augmented_info.csv"))
    gn.fn <- file.path(datadir, paste0(s, "_gene_augmented_info.csv"))
    regex <- if (s == "mouse") mouse_regex else human_regex

    gr <- .load_gtf(regex, datadir)
