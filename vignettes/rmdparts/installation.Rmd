...
function which creates this directory and copies over a `meta.yaml` into that
directory which specifies the files which are expected to be found there:
...
In order to augment these files with richer annotation such as the ensembl gene
identifiers or gene biotypes, for instance.
...
run the `create_augmented_feature_info()` to create the feature-level metadata
tables that provide richer information for the features in the datasets:
...
This function simply look at your `$ARCHS4DIR` to ensure that the required
...
finished successfully, a call to `archs4_local_data_dir_validate()` will simply
return `TRUE`.
